...
use crate::{
    extensions::mintable::FungibleMintable,
    fungible::{emit_approve, emit_mint, emit_transfer, FungibleToken, FungibleTokenError},
};
...
impl FungibleMintable for Base {
    type Impl = Base;
...
impl Base {
    ...
    pub fn update(e: &Env, from: Option<&Address>, to: Option<&Address>, amount: i128) {
        ...
        if let Some(account) = from {
            let mut from_balance = Base::balance(e, account.clone());
        ...
        if let Some(account) = to {
            ...
            let to_balance = Base::balance(e, account.clone()) + amount;
