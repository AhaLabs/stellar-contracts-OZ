...
#[contracttrait(default = BlockList, is_extension = true, extension_required = true)]
...
impl<T: Administratable, N: FungibleBlockList> FungibleBlockList for AdministratableExt<T, N> {
    type Impl = N;

    fn block_user(e: &Env, user: Address, operator: Address) {
        T::require_admin(e);
        Self::Impl::block_user(e, user.clone(), operator);
        emit_user_blocked(e, &user);
    }

    fn unblock_user(e: &Env, user: Address, operator: Address) {
        T::require_admin(e);
        Self::Impl::unblock_user(e, user.clone(), operator);
